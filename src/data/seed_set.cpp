	...
	data_((size_t)pow(1llu<<Reduction::reduction.bit_size(), shapes[0].length_))
{
	...
	enum_seeds(&seqs, v, seqs.partition(1), 0, 1, &no_filter, SeedEncoding::CONTIGUOUS, skip, true, false);
...
HashedSeedSet::HashedSeedSet(SequenceSet &seqs, const std::vector<bool>* skip)
{
	...
	enum_seeds(&seqs, v, seqs.partition(1), 0, shapes.count(), &no_filter, SeedEncoding::HASHED, skip, false, false);
